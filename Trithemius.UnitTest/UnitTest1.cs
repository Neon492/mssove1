...
namespace Trithemius.UnitTest
{
    ...
    public class UnitTest1
    {
        ...
        public void TestMethod1()
        {
            try
            {
                ...
                for (int i = 0; i < 20; i++)
                {
                    ...
                    int columnKey = 5 + rnd.Next() % 10;
                    string text = texts[i%texts.Length];
                    Console.WriteLine(@"Тест #:               " + i);
                    Console.WriteLine(@"Ключ:                 " + key);
                    Console.WriteLine(@"Количество колонок:   " + columnKey);
                    Console.WriteLine(@"Исходный текст:       " + text);
                    cryptography.SetKey(key, columnKey);
            ...
            {
                Console.WriteLine(@"Используются символы не содержащиеся в алфавите (" + exception.Character + ")");
                Assert.IsTrue(false);
            }
            catch (TrithemiusCryptography.EmptyKeyException)
            {
                Console.WriteLine(@"Ключ не содержит символов");
                Assert.IsTrue(false);
            }
            catch (Exception)
            {
        ...
        public void TestMethod2()
        {
            try
            {
                Console.WriteLine(@"Шифр Трисемуса (модификация)");
                Console.WriteLine();
                TrithemiusCryptography cryptography = new TrithemiusCryptography();
                for (int i = 0; i < 20; i++)
                {
